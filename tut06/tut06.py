def attendance_report():
    ...
    date_list = list({datetime.strptime(str(i).split(" ")[0],"%m/%d/%Y %H:%M:%S.%f").date() for i in df['Timestamp']  if datetime.strptime(str(i).split(" ")[0],"%m/%d/%Y %H:%M:%S.%f").strftime('%a') in ['Mon','Thu']})
    date_list.sort()
